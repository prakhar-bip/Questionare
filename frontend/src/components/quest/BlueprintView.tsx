      ...
      <div className="panel q-rise overflow-hidden">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b-2 border-border bg-primary px-6 py-5 text-primary-foreground">
          <div>
            ...
            <h2 className="font-display text-3xl font-extrabold">{b?.title || "Project Plan"}</h2>
          </div>
          <div className="flex items-center gap-3">
            <span className="mono-label rounded-full bg-primary-foreground/20 px-3 py-1 text-xs font-bold text-primary-foreground border border-primary-foreground/30">
              ✓ BLUEPRINT READY
            </span>
        ...
        <div className="grid gap-4 px-6 py-5 lg:grid-cols-2">
          <div className="space-y-3 text-sm">
            ...
            <div className="rounded-xl border-2 border-border bg-background p-3">
              ...
              <p className="text-xs leading-relaxed">{overview.problemStatement}</p>
            </div>
            <div className="rounded-xl border-2 border-border bg-background p-3">
              <div className="mono-label mb-1">proposed solution</div>
              <p className="text-xs leading-relaxed">{overview.proposedSolution}</p>
            </div>
          </div>
          <div className="space-y-3">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="rounded-xl border-2 border-border bg-background p-3">
                ...
                <p className="text-xs">{overview.targetUsers}</p>
              </div>
              <div className="rounded-xl border-2 border-border bg-background p-3">
                <div className="mono-label mb-1">expected impact</div>
                <p className="text-xs">{overview.expectedImpact}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      {onGeneratePrototype && (
        <div className="panel q-rise border-2 border-accent bg-gradient-to-r from-accent/15 via-background to-accent/5 p-6 sm:p-8 text-center space-y-4">
          <span className="mono-label rounded-full bg-accent/20 px-3 py-1 font-bold text-accent">
            NEXT PHASE: THEME & PROTOTYPE FORGE
          </span>
          <h3 className="font-display text-2xl sm:text-3xl font-extrabold tracking-tight">
            Ready to choose a design theme and build your prototype?
          </h3>
          <p className="mx-auto max-w-xl text-sm sm:text-base text-muted-foreground leading-relaxed">
            Sarthi will ask you to select a tailored visual theme (Neo-Brutalism, Modern Minimal SaaS, Cyberpunk, Glassmorphism, etc.), then synthesize your blueprint into an interactive in-browser prototype and a multi-file starter codebase.
          </p>
          <div className="pt-2 flex flex-wrap justify-center gap-3">
            <button
              ...
              className="btn-brutal flex items-center gap-2.5 bg-accent px-6 py-3 text-base font-extrabold text-accent-foreground shadow-lg hover:scale-105 transition-transform cursor-pointer"
            >
              <span className="text-xl">🎨</span>
              <span>{isGeneratingPrototype ? "Manifesting Prototype with Sarthi AI..." : "Select Theme & Build Prototype →"}</span>
            </button>
          </div>
        </div>
      )}
