    ...
    try {
      const res = await fetch(`${backendUrl}/api/gateway/generate-json`, {
        ...
        body: JSON.stringify({
          system: opts.system,
          prompt: opts.prompt,
          agent_name: opts.agentName || "Sarthi JSON Gateway Agent",
        }),
      });
...
export async function generateText(opts: {
    ...
    try {
      const res = await fetch(`${backendUrl}/api/gateway/generate-text`, {
        ...
        body: JSON.stringify({
          ...
          agent_name: opts.agentName || "Sarthi Code Architect Agent",
        }),
      });
  ...
  try {
    ...
    const jsonRes = await generateJson<{
      ...
      agentName: opts.agentName || "Sarthi Code Architect Agent (JSON Fallback)",
    });
