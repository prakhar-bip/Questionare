...
export function RepoForgeModal({
  ...
  const defaultRepoName = `sarthi-${(prototype?.title || blueprint?.title || "app")
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "")
    .slice(0, 36)}`;
  ...
  const [description, setDescription] = useState(
    prototype?.tagline || prototype?.architectureSummary || "Scaffolded with Sarthi AI Charioteer"
  );
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      const savedToken = localStorage.getItem("sarthi_github_token");
      if (savedToken) {
        setToken(savedToken);
      }
    }
  ...
  const handlePushToGitHub = async (e: React.FormEvent) => {
    ...
    if (rememberToken) {
      localStorage.setItem("sarthi_github_token", token.trim());
    } else {
      ...
      >
                ...
                <form onSubmit={handlePushToGitHub} className="space-y-4">
                  <div className="rounded-xl border border-blue-100 bg-blue-50/50 p-3.5 text-xs text-slate-700 flex items-start gap-2.5">
                    ...
                    <p className="leading-relaxed">
                      Sarthi will connect to GitHub via the official REST API, create the repository under your account, commit the complete architecture tree, and set up CI/CD workflows.
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="High-performance system scaffolded by Sarthi AI"
                  ...
                  <div className="rounded-2xl border border-slate-200/80 bg-slate-50/70 p-4 space-y-2.5">
                    <div className="flex items-center justify-between">
                      ...
                      <a
                        href="https://github.com/settings/tokens/new?scopes=repo&description=Sarthi+AI+Forge"
                        target="_blank"
                        rel="noreferrer"
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 bg-slate-50/50 px-6 py-4">
          <span className="text-[11px] text-slate-500">
            Powered by Sarthi AI Architecture Engine
          </span>
          <button
            type="button"
            onClick={onClose}
